...
class WhenRequestingFocus : AndroidContext() {
	...
	fun `then the request is granted using the inner listener`() {
		Assertions.assertThat(result.onAudioFocusChangeListener.toString()).isEqualTo(request.onAudioFocusChangeListener.toString())
	}

	@Test
	fun `then other events are forwarded correctly`() {
		Assertions.assertThat(audioFocusEvents).containsOnly(AudioManager.AUDIOFOCUS_LOSS)
	}
}
