...
class WhenBuildingTheNotification : AndroidContext() {
	...
	override fun before() {
		...
		val npBuilder = NowPlayingNotificationBuilder(
			...
			mockk {
				every { promiseImageBytes(libraryId, any<ServiceFile>()) } returns Promise(
					expectedBitmap
				)
			},
	...
	fun thenTheNotificationHasAPauseButton() {
		assertThat(builder!!.mActions.map { a -> a.title }).containsOnlyOnce("30dSyjMHKm0")
	}

	@Test
	fun thenTheNotificationHasAPreviousButton() {
		assertThat(builder!!.mActions.map { a -> a.title }).containsOnlyOnce("mMIuesOgcME")
	}

	@Test
	fun thenTheNotificationHasANextButton() {
		assertThat(builder!!.mActions.map { a -> a.title }).containsOnlyOnce("DbxWr82p")
	}
