...
class WhenBuildingTheEditClientSettingsActivityIntent {
    ...
    @Before
    fun before() {
        val editClientSettingsActivityIntentBuilder =
            IntentBuilder(ApplicationProvider.getApplicationContext())
        returnedIntent = editClientSettingsActivityIntentBuilder.buildLibrarySettingsIntent(
            LibraryId(13)
        )
    }
    ...
    fun thenTheIdInTheIntentIsSetToTheLibraryId() {
        Assertions.assertThat(
            returnedIntent?.getParcelableExtra(
                destinationProperty,
                cls<ConnectionSettingsScreen>()
            )?.libraryId
