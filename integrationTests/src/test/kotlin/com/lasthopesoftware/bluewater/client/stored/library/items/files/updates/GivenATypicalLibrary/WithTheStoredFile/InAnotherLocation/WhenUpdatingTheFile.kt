...
class WhenUpdatingTheFile {
	...
	companion object {
		private var storedFile = Lazy {
            ...
            val fakeLibraryRepository = mockk<ProvideLibraries>  {
				every { promiseLibrary(LibraryId(14)) } returns Library(id = 14).toPromise()
			}
            ...
            every {
                lookupStoredFilePaths.promiseStoredFileUri(
                    LibraryId(14),
                    ServiceFile("4")
                )
            } returns Promise(
