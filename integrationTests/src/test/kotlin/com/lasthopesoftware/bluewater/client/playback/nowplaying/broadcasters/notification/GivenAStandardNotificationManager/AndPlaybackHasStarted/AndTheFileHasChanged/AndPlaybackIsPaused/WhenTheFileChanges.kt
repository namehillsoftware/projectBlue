...
class WhenTheFileChanges : AndroidContext() {
	...
	companion object {
		...
		private val notificationController =
            mockk<ControlNotifications>(relaxUnitFun = true, relaxed = true)
		private val notificationContentBuilder = mockk<BuildNowPlayingNotificationContent>()
	}

	override fun before() {
		every {
            notificationContentBuilder.promiseLoadingNotification(
                LibraryId(libraryId),
                any()
            )
        } returns com.lasthopesoftware.resources.notifications.FakeNotificationCompatBuilder.newFakeBuilder(
		...
		every {
            ...
            FakeNotificationCompatBuilder.newFakeBuilder(ApplicationProvider.getApplicationContext(),
            firstNotification
        ))
		every {
            notificationContentBuilder.promiseNowPlayingNotification(
            ...
            com.lasthopesoftware.resources.notifications.FakeNotificationCompatBuilder.newFakeBuilder(
                ApplicationProvider.getApplicationContext(),
            secondNotification
        ))
		...
		val nowPlayingRepository =
            com.lasthopesoftware.bluewater.client.playback.nowplaying.FakeNowPlayingRepository(
                nowPlaying
            )
