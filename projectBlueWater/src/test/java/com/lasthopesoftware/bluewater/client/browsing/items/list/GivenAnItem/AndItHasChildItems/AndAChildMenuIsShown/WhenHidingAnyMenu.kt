package com.lasthopesoftware.bluewater.client.browsing.items.list.GivenAnItem.AndItHasChildItems.AndAChildMenuIsShown
...
private const val libraryId = 246
private const val rootItemId = 377
private const val childItemId = 974
...
class WhenHidingAnyMenu {
	...
	companion object {
		...
		fun act() {
			...
			viewModel.items.value[1].showMenu()
			isMenuHidden = viewModel.hideAnyShownMenus()
		}
	}

	@Test
	fun `then it is indicated that a menu was hidden`() {
		assertThat(isMenuHidden).isTrue
	}

	@Test
	fun `then the menu is not shown`() {
		assertThat(viewModel.items.value[3].isMenuShown.value).isFalse
	}
