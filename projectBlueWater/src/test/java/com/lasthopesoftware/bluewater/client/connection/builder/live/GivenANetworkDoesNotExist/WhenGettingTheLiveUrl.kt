...
class WhenGettingTheLiveUrl {
	...
	private val urlProvider by lazy {
		...
		every { urlProviderBuilder.promiseBuiltUrlProvider(LibraryId(23)) } returns ServerConnection(baseUrl = URL("http://test-url")).toPromise()

		val liveUrlProvider = LiveServerConnectionProvider(
			mockk {
				every { isNetworkActive } returns false
		  	},
			urlProviderBuilder
		)
