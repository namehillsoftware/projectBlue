package com.lasthopesoftware.bluewater.client.settings.GivenALibraryId.AndLibrarySettingAreLoaded.AndTheServerTypeIsNotChanged
...
class `When saving the connection type` {
	...
	companion object {
		private const val libraryId = 927
	}
	...
	fun act() {
		...
		repository.isLoading.mapNotNull().subscribe(isLoadingStates::add).toCloseable().use {
			repository.isChanged.mapNotNull().subscribe(isChangedStates::add).toCloseable().use {
				...
				repository.serverType.value = Library.ServerType.Subsonic
	...
	fun `then the is loading states are correct`() {
		assertThat(isLoadingStates).containsExactly(false, true, false)
	}
	...
	@Test
	fun `then the library is correct`() {
