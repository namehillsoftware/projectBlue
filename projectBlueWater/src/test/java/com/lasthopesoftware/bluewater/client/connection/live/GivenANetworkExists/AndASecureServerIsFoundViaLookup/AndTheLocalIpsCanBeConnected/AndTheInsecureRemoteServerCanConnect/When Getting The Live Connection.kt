package com.lasthopesoftware.bluewater.client.connection.live.GivenANetworkExists.AndASecureServerIsFoundViaLookup.AndTheLocalIpsCanBeConnected.AndTheInsecureRemoteServerCanConnect
...
class `When Getting The Live Connection` {
	...
	private val services by lazy {
		...
		every { connectionSettingsLookup.lookupConnectionSettings(LibraryId(777)) } returns MediaCenterConnectionSettings(
			accessCode = "gooPc"
		).toPromise()
	...
	fun `then the base url is correct`() {
		assertThat(liveServerConnection?.serverConnection?.baseUrl.toString()).isEqualTo("http://1.2.3.4:143")
	}
}
