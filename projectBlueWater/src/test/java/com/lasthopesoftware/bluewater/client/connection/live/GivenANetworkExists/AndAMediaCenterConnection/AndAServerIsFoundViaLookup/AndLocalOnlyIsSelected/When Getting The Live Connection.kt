package com.lasthopesoftware.bluewater.client.connection.live.GivenANetworkExists.AndAMediaCenterConnection.AndAServerIsFoundViaLookup.AndLocalOnlyIsSelected
...
class `When Getting The Live Connection` {
	...
	private val mutt by lazy {
		...
		val urlScanner = LiveServerConnectionProvider(
			...
			serverLookup,
			mockk {
				every { promiseConnectionSettings(LibraryId(6)) } returns MediaCenterConnectionSettings(
					accessCode = "gooPc",
					isLocalOnly = true
				).toPromise()
			},
	...
	@BeforeAll
	fun act() {
		serverConnection = mutt.promiseLiveServerConnection(LibraryId(6)).toExpiringFuture().get()
	}
