...
class WhenGettingTheStoredFilePath {
	companion object {
		private val filePath by lazy {
			...
			every { libraryProvider.getLibrary(LibraryId(445)) } returns Promise(
				Library(_id = 445, _syncedFileLocation = Library.SyncedFileLocation.EXTERNAL)
			)

			val externalFiles = mockk<SharedStoredFilePathLookup>()
			every { externalFiles.promiseSharedStoredFilePath(LibraryId(445), ServiceFile(31)) } returns Promise("omit")

			val privateFiles = mockk<PrivateStoredFilePathLookup>()
			every { privateFiles.promiseStoredFilePath(LibraryId(445), ServiceFile(31)) } returns Promise("awkward")

			val storedFilePaths = StoredFilePathsLookup(libraryProvider, privateFiles, externalFiles)
			storedFilePaths
				.promiseStoredFilePath(LibraryId(445), ServiceFile(31))
				.toFuture()
				.get()
		}
	}
	...
	fun thenTheFilePathIsCorrect() {
		assertThat(filePath).isEqualTo("omit")
	}
}
