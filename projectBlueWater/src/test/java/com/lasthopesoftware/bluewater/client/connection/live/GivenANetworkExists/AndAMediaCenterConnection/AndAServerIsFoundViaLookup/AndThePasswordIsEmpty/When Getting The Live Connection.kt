package com.lasthopesoftware.bluewater.client.connection.live.GivenANetworkExists.AndAMediaCenterConnection.AndAServerIsFoundViaLookup.AndThePasswordIsEmpty
...
class `When Getting The Live Connection` {
	...
	private val mutt by lazy {
		...
		val urlScanner = LiveServerConnectionProvider(
			...
			serverLookup,
			mockk {
				every { promiseConnectionSettings(LibraryId(56)) } returns MediaCenterConnectionSettings(
					accessCode = "gooPc",
					userName = "user",
					password = ""
				).toPromise()
	...
	@BeforeAll
	fun act() {
		serverConnection = mutt.promiseLiveServerConnection(LibraryId(56)).toExpiringFuture().get()
	}
