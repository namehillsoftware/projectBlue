...
class WhenTheFileChanges : AndroidContext() {
    ...
    override fun before() {
		...
		val notificationContentBuilder = mockk<BuildNowPlayingNotificationContent> {
			...
			every { promiseNowPlayingNotification(LibraryId(libraryId), any(), any()) } returns newFakeBuilder(context, Notification()).toPromise()
			every { promiseNowPlayingNotification(LibraryId(libraryId), ServiceFile(serviceFileId), any()) } returns secondNotificationPromise
		}
