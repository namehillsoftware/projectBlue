package com.lasthopesoftware.bluewater.client.access.subsonic.GivenASubsonicConnection.AndTheUserHasTheAdminRole
...
class `When Checking Authentication` {
	...
	private val isReadOnly by lazy {
		val httpConnection = FakeHttpConnection().apply {
			mapResponse(TestUrl.withSubsonicApi().addPath("getUser").addParams("username=XjEdXrsEUA").addParams("f=json")) {
    ...
    "user": {
      ...
      "shareRole": "false"
		...
		val access = LiveSubsonicConnection(
			SubsonicConnectionDetails(TestUrl, "XjEdXrsEUA", "bNI1AuryTD"),
