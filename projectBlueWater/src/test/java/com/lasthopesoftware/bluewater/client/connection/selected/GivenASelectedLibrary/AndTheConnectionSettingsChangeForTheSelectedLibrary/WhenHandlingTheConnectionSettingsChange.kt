...
class WhenHandlingTheConnectionSettingsChange : AndroidContext() {
	...
	override fun before() {
		...
		every { mockSelectedLibraryIdentifierProvider.selectedLibraryId } returns LibraryId(4)
	...
	fun thenAnUpdateIsSentOnTheMessageBus() {
		assertThat(messageBus.recordedIntents.map { it.action })
			.containsOnly(SelectedConnectionSettingsChangeReceiver.connectionSettingsUpdated)
	}
}
