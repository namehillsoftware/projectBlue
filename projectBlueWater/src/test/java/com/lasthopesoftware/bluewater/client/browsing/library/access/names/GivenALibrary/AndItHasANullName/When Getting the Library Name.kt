package com.lasthopesoftware.bluewater.client.browsing.library.access.names.GivenALibrary.AndItHasANullName
...
class `When Getting the Library Name` {
		...
		LibraryNameLookup(
			mockk {
				every { promiseLibrarySettings(LibraryId(libraryId)) } returns LibrarySettings(
					libraryName = null,
					connectionSettings = StoredMediaCenterConnectionSettings(accessCode = "4TW8rdEq")
				).toPromise()
			},
		)
	}
	...
	fun `then the library name is correct`() {
		assertThat(name).isEqualTo("4TW8rdEq")
	}
}
