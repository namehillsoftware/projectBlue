package com.lasthopesoftware.bluewater.client.stored.library.items.files.destination.GivenAStoredFile.AndAContentUri.AndTheFileExists.AndTheDownloadIsComplete
...
private const val storedMediaId = 79L
...
class `When Getting the File Output Stream` {
	...
	fun `then the output stream is correct`() {
		Assertions.assertThat(outputStream).isNull()
	}
}
