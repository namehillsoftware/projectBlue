...
class `When Getting The Uri Using Metadata` : AndroidContext() {
		...
		private val uriProvider by lazy {
			MetadataMediaFileUriProvider(
				...
				AND COALESCE(${MediaStore.Audio.AlbumColumns.ALBUM}, "") = ?""",
					...
					every { openFileDescriptor(ContentUris.withAppendedId(MediaCollections.ExternalAudio, 520), "r") } returns mockk(relaxUnitFun = true)
				}
			)
		}
	}
	...
	fun `then the uri is correct`() {
		assertThat(uri).isEqualTo(ContentUris.withAppendedId(MediaCollections.ExternalAudio, 520))
	}
}
