package com.lasthopesoftware.bluewater.client.connection.libraries.GivenALibrary.AndGettingALiveUrlThrowsAnException.AndAConnectionIsStillAlive;
...
public class WhenGettingATestedLibraryConnection {
	...
	public static void before() throws InterruptedException, ExecutionException {
		...
		final FuturePromise<IConnectionProvider> futureConnectionProvider = new FuturePromise<>(libraryConnectionProvider
			...
			.eventually(
				c -> libraryConnectionProvider.promiseTestedLibraryConnection(libraryId).updates(s -> {
					statuses.add(s);
					return Unit.INSTANCE;
				}),
				c -> libraryConnectionProvider.promiseTestedLibraryConnection(libraryId).updates(s -> {
