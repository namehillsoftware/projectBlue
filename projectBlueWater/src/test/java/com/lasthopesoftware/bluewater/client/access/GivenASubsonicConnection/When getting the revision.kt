...
class `When getting the revision` {
	private val mut by lazy {
		val httpConnection = FakeHttpConnection().apply {
			mapResponse(TestUrl.withSubsonicApi().addPath("getIndexes").addParams("f=json")) {
	...
	@BeforeAll
	fun act() {
		revision = mut.promiseRevision().toExpiringFuture().get()!!
	}

	@Test
	fun `then the revision is correct`() {
		assertThat(revision).isEqualTo(1743978082001)
	}
}
