 ...
 */
@Composable
fun rememberDirectionalScrollConnectedScaler(max: Float, min: Float) = rememberSaveable(saver = DirectionalScrollConnectedScaler.Saver) {
	DirectionalScrollConnectedScaler(max, min)
}

class DirectionalScrollConnectedScaler private constructor(private val max: Float, private val min: Float, initialDistanceTraveled: Float) : BoundedScrollConnection {
	...
	object Saver : androidx.compose.runtime.saveable.Saver<DirectionalScrollConnectedScaler, Triple<Float, Float, Float>> {
		override fun restore(value: Triple<Float, Float, Float>): DirectionalScrollConnectedScaler =
			DirectionalScrollConnectedScaler(value.first, value.second, value.third)

		override fun SaverScope.save(value: DirectionalScrollConnectedScaler): Triple<Float, Float, Float> =
