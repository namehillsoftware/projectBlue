package com.lasthopesoftware.bluewater.client.library.items.media.files.stored.download.specs.GivenAStoredFileReadError;
...
public class WhenProcessingTheQueue {
	...
	public static void before() {
		...
		when(storedFileJobs.promiseDownloadedStoredFile(new StoredFileJob(new ServiceFile(7), new StoredFile().setLibraryId(4).setServiceId(7))))
			.thenAnswer(a -> new Promise<>(new StoredFileReadException(mock(File.class), a.<StoredFileJob>getArgument(0).getStoredFile())));
