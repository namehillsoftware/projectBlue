package com.lasthopesoftware.bluewater.client.sync.library.items.files.download.specs.GivenAGenericStoredFileJobException;
...
public class WhenProcessingTheQueue {
	...
	public static void before() {
		final StoredFileDownloader storedFileDownloader = new StoredFileDownloader(job -> {
			if (Arrays.asList(7, 2).contains(job.getServiceFile().getKey()))
				return Observable.concat(
					...
					Observable.error(new StoredFileJobException(job.getStoredFile(), new Exception())));
