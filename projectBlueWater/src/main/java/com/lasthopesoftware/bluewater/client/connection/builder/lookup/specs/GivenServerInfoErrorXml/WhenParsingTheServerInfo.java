package com.lasthopesoftware.bluewater.client.connection.builder.lookup.specs.GivenServerInfoErrorXml;
...
public class WhenParsingTheServerInfo {
		...
		when(serverInfoXml.promiseServerInfoXml(any()))
			.thenReturn(new Promise<>(Xmlwise.createXml(
				...
				"<Response Status=\"Error\">\n" +
					"<msg>Keyid gooPc not found.</msg>" +
				"</Response>")));

		final ServerLookup serverLookup = new ServerLookup(serverInfoXml);

		try {
			new FuturePromise<>(serverLookup.promiseServerInformation(new LibraryId(14))).get();
	...
	public void thenAServerDiscoveryExceptionIsThrownWithTheCorrectMessage() {
		assertThat(exception.getMessage()).contains("Keyid gooPc not found.");
	}
}
