...
object UrlBuilder {
	...
	fun URL.addParams(vararg params: String): URL {
		val urlBuilder = StringUtil.borrowBuilder()
		try {
			...
			for (i in params.indices) {
				...
				urlBuilder.append(encodeParameter(param.substring(0, equalityIndex))).append('=')
					.append(encodeParameter(param.substring(equalityIndex + 1)))
			}

			return URL(urlBuilder.toString())
		} finally {
		    StringUtil.releaseBuilder(urlBuilder)
		}
	}
