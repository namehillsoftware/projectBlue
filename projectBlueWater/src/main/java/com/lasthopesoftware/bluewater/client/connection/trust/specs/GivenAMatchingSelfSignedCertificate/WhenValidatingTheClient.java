...
public class WhenValidatingTheClient {
	...
	public static void before() throws CertificateException, IOException {
		...
		final byte[] certBytes = Hex.decode("1b6fae967b4a13192d3b65bba33cf7fc510df456");
		final SelfSignedTrustManager selfSignedTrustManager = new SelfSignedTrustManager(certBytes, fallbackTrustManager);

		try {
			selfSignedTrustManager.checkServerTrusted(new X509Certificate[] { cert }, null);
	...
	@Test
	public void thenTheFallbackTrustManagerIsNotCalled() throws CertificateException {
		verify(fallbackTrustManager, never()).checkServerTrusted(any(), any());
	}
}
