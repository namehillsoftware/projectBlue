...
import com.lasthopesoftware.bluewater.client.library.items.media.files.uri.ProvideFileUrisForLibrary;
...
public class MediaFileUriProvider implements IFileUriProvider, ProvideFileUrisForLibrary {
	...
	public Promise<Uri> promiseFileUri(ServiceFile serviceFile) {
		return promiseUri(libraryIdentifierProvider.getSelectedLibraryId(), serviceFile);
	}

	@NotNull
	@Override
	public Promise<Uri> promiseUri(@NotNull LibraryId libraryId, @NotNull ServiceFile serviceFile) {
		if (!externalStorageReadPermissionsArbitrator.isReadPermissionGranted())
			return Promise.empty();

		return mediaQueryCursorProvider
			...
			.then(cursor -> {
				...
				try {
					...
					if (!isSilent) {
						...
						broadcastIntent.putExtra(mediaFileFoundLibraryId, libraryId.getId());
