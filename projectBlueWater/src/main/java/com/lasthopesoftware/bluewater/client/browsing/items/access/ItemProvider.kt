...
class ItemProvider(private val connectionProvider: ProvideLibraryConnections) : ProvideItems {
    override fun promiseItems(libraryId: LibraryId, itemKey: Int): Promise<List<Item>> =
    	connectionProvider
			.promiseLibraryConnection(libraryId)
			.eventually { c -> c?.promiseResponse(LibraryViewsProvider.browseLibraryParameter, "ID=$itemKey", "Version=2") ?: Promise.empty() }
