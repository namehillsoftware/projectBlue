...
class DeviceEncryptedStringGuard(
	...
	override fun promiseDecryption(encryptedString: EncryptedString): Promise<String> = ThreadPools.compute.preparePromise {
		...
		cipher.init(Cipher.DECRYPT_MODE, getOrCreateSecretKey(), IvParameterSpec(ivBytes))
	...
	private fun getOrCreateSecretKey(): SecretKey {
		val alias = encryptionKeyLookup.encryptionKey // Use the encryption key as the alias instead to provide some security
		...
		return KeyGenerator.getInstance(configureEncryption.algorithm).apply {
			init(spec)
		}.generateKey()
	}
}
