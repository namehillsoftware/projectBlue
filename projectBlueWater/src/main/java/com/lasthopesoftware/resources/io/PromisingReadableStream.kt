...
class PromisingChannel(pipeSize: Int = DEFAULT_PIPE_SIZE) : PromisingReadableStream {
	...
	val writableStream: PromisingOutputStream<*> by lazy { PromisingWritableStream() }
	...
	private inner class PromisingWritableStream() : PromisingOutputStream<PromisingWritableStream>, ImmediateResponse<Unit, PromisingWritableStream> {
		override fun promiseWrite(buffer: ByteArray, offset: Int, length: Int): Promise<PromisingWritableStream> =
			promiseReceive(buffer, offset, length).then(this)

		override fun flush(): Promise<PromisingWritableStream> = awaitSpace().then(this)

		override fun close() {
			receivedLast()
		}

		override fun respond(resolution: Unit?): PromisingWritableStream = this
	}
